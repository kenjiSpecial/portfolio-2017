...
export default class Particles extends THREE.Points  {
    constructor(params) {
        ...
        let totalLength = params.number || 2200; //params.geometry.vertices.length;
        let positions = new Float32Array(totalLength * 3);
        let positions2 = new Float32Array(totalLength * 3);
        let positions3 = new Float32Array(totalLength * 3);
        let positions4 = new Float32Array(totalLength * 3);
        let speeds = new Float32Array(totalLength * 4);

        let faces = params.mainGeometry.faces;
        let subFaces = params.subGeometry.faces;

        for(let ii = 0; ii < totalLength; ii++){
            let index1 = THREE.Math.randInt(0, faces.length - 1);
            let index2 = THREE.Math.randInt(0, subFaces.length - 1);

            setValues( positions, params.mainGeometry.vertices, faces, index1, ii);
            setValues( positions2, params.mainGeometry.vertices, faces, index1, ii);
            setValues( positions3, params.subGeometry.vertices, subFaces, index2, ii);
            setValues( positions4, params.subGeometry.vertices, subFaces, index2, ii);
        ...
        let positionAttributre3 = new THREE.BufferAttribute(positions3, 3);
        let positionAttributre4 = new THREE.BufferAttribute(positions4, 3);
        ...
        geometry.addAttribute("position3", positionAttributre3);
        geometry.addAttribute("position4", positionAttributre4);
        ...
        let uniforms = {
                ...
                uActiveColor : {value : new THREE.Color("#196284")},
                uActiveRollColor : {value : new THREE.Color("#2091BE")},
                uTranslate : {value: 0}
            };
        let mat = new THREE.ShaderMaterial({
            ...
            depthTest : false,
            vertexShader : glslify('../../shaders/keyObjectParticle3.vert'),
            fragmentShader : glslify('../../shaders/keyObjectParticle3.frag')
        });
